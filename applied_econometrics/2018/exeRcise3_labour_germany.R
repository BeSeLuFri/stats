...
#No 1.1
data <- read.csv2(file = "data/labour_germany2.csv",header = TRUE)

#overview over the data
names(data)[4] <- "CAPITAL"
names(data)
...
par(mfrow=c(2,2))
for (q in names(data)){
  eval(parse(text=(paste("data$",q," %>% hist(xlim = c(0,2000),breaks = c(seq(0,50000,100)),main = '",q,"') ",sep=""))))
}
...
plot(x= data$LABOUR_LOG_PREDICTED, 
     y = resid(reg), 
     type = "p",  
     xlim = NULL, 
     ylim = NULL,
     log = "", 
     main = "PREDICTED VS. RESIDUAL PLOT", 
     sub = NULL, 
     xlab = "Predicted LABOUR_LOG", 
     ylab = "RESIDUALS")
...
library(lmtest)
bptest(formula = resid(reg) ~ WAGE_LOG + OUTPUT_LOG + CAPITAL_LOG , 
       data=data ,
       studentize = TRUE)


# No. 4 Heteroskedasticity-Robust Inference
